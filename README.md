...
Custom stacks are saved to the browser's `localStorage`, so they persist
between visits on the same browser/device.
...
- `index.html` — markup for every screen (start, game setup, stack
  editor, play, results, wiki, options); screens are shown/hidden via a
  `.active` class.
...
- `js/storage.js` — localStorage persistence for custom stacks.
